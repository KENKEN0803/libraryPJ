...
<fieldset>
    <form method=POST action="mem_update">
        <label>아이디</label><input type="text" name="mem_id" id="mem_id" value="<%=session.getAttribute("member")%>"
                                 readonly/><br>
        <label>패스워드</label><input type="text" name="mem_pass" value="${m.get(0).mem_pass}"><Br>
        <label>이름</label><input type="text" name="mem_name" id="mem_name" required value="${m.get(0).getMem_name()}"
                                onchange="nameCheck(this)" required><Br>
