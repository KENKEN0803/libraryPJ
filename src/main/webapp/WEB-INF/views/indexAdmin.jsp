...
<div class="total">
	...
	<div style="min-height: 405px;">
		...
		<form action="indexAdmin" method="get" style="margin-top: 90px;">
			<a style="float: left; font-size: 20px; padding-top: 8px; margin-left: 37%; color: black;">회&nbsp;&nbsp;&nbsp;&nbsp;원 ID : &nbsp;</a>
		...
		<div class="table" style="margin-left:5%; margin-right:5%; float: left; margin-top: 50px;">
			<table>
				<thead>
				<tr>
					<th colspan="8"> 대여 상태 <%=mem.size() %></th>
				</tr>
				<tr>
					<th>BOOK ID</th>
					<th>책 이름</th>
				...
				<tr>
					<td><%=item.get("b_id")%></td>
					<td><%=item.get("b_name")%></td>
					<td><%=item.get("b_author")%></td>
					<td><%=item.get("b_publisher")%></td>
					<td><%=item.get("rent_date")%></td>
					<td><%=item.get("return_date")%></td>
					<td>
						<%
							if(extendCheckNum == 1 && item.get("book_b_id") == null) {
						%>
						<form onsubmit="return confirm('연장 하시겠습니까?');" method="Post" action="extendReturnDateSub">
